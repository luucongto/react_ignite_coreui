...
export const ${changeCase.pascalCase(name)}Selector = (state) =>  state.${changeCase.camelCase(
  name,
)}
/* ------------- Initial State ------------- */
...
export const ${changeCase.camelCase(
  name,
)}Request = state => state.merge({ fetching: true, error: null, data: null })

export const ${changeCase.camelCase(
  name,
)}Success = (state, { data }) => state.merge({ fetching: false, error: null, data: data })

export const ${changeCase.camelCase(
  name,
)}Failure = (state, { error }) => state.merge({ fetching: false, error, data: null })
...
let tempSaga = `import { call, put } from 'redux-saga/effects'
...
export function * ${changeCase.camelCase(name)} (api, {params}) {
  try {
    ...
    if (res.data) { 
      yield put(${changeCase.pascalCase(name)}Actions.${changeCase.camelCase(
  name,
)}Success(res.data)) 
    } else {
      yield put(${changeCase.pascalCase(name)}Actions.${changeCase.camelCase(name)}Failure(res))
    }
  } catch (error) {
    yield put(${changeCase.pascalCase(name)}Actions.${changeCase.camelCase(
  name,
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      if (line === TypesLine) {
        ...
        newFileStrs.push(
          `import { ${changeCase.pascalCase(name)}Types } from '../Redux/${changeCase.pascalCase(
            name,
          )}Redux'`,
        )
        ...
        inserted = true
        newFileStrs.push(
          `import { ${changeCase.camelCase(name)} } from './${changeCase.pascalCase(name)}Saga'`,
        )
      } else if (line.trim() === connect) {
        newFileStrs.push(line)
        inserted = true
        newFileStrs.push(
          `    takeLatest(${changeCase.pascalCase(name)}Types.${changeCase
            .snakeCase(name)
            .toUpperCase()}_REQUEST, ${changeCase.camelCase(name)}, api.${changeCase.camelCase(
            name,
          )}),`,
        )
      } else {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      if (line.trim() === reducerLine) {
        ...
        inserted = true
        newFileStrs.push(
          `    ${changeCase.camelCase(name)}: require('./${changeCase.pascalCase(
            name,
          )}Redux').reducer,`,
        )
      } else {
...
try {
  ...
  console.log(
    `DONE:`,
    `./src/Redux/${changeCase.pascalCase(name)}Redux.js`,
    `./src/Sagas/${changeCase.pascalCase(name)}Sagas.js`,
  )
} catch (e) {
  console.log(e)
}
